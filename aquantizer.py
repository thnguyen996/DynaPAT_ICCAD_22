...
def dequantized_model(quantized_state_dict, qparams):
    with torch.no_grad():
        for name, value in quantized_state_dict.items():
            if ( "weight" in name ) and ( "bn" not in name ):
                scale = qparams[name + ".scale"]
                zero_point = qparams[name + ".zero_point"]
                dequantized_weight = linear_dequantize(value, scale, zero_point)
                value.copy_(dequantized_weight)
    return quantized_state_dict
# Testing 
if __name__ == '__main__':
    param_fp = torch.randn(5, 3, 3, 3)
    out = dorefa_quantize_param(param_fp, 8)
