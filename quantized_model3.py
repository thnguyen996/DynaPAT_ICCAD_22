...
from weight_quantized_conf_method3 import inject_error
import argparse
import copy
import distiller
import logging
...
# Model
    ...
    index_sort = np.load(f"./state_stats/{args.model}-state-stats.npy")

    # # Resume 
    # if args.model == "LeNet":
    #     error_11_pd = error_11_pd.loc[:, 8:]
    #     error_10_pd = error_10_pd.loc[:, 8:]
    # elif args.mode == "Inception":
    #     error_11_pd = error_11_pd.loc[:, 5:]
    #     error_10_pd = error_10_pd.loc[:, 5:]
            ...
            for it in range(iteration):
                ...
                for name, weight in tqdm(net.named_parameters(), desc="Executing method:", leave=False):
                    if ( "weight" in name ) and ( "bn" not in name ) and ("shortcut" not in name):
                        # level = "bit"
                        state_order = index_sort[:, layer_index]
                        mlc_error_rate = {"error_level3" : error_11, "error_level2": error_10}
                        if args.method == "proposed_method":
                            error_weight = proposed_method(weight, mlc_error_rate, args.num_bits, tensors, state_order)
...
def proposed_method(weight, mlc_error_rate, num_bits, tensors, state_order):
    ...
    state = ("00", "01", "10", "11")
    level2 = state[state_order[1]]
    level3 = state[state_order[0]]
    level4 = state[state_order[3]]
    shape = weight.shape
    weight = weight.view(-1).detach().cpu().numpy()
    orig_weight = np.copy(weight)
    error_weight = inject_error(weight, orig_weight, mlc_error_rate["error_level3"], level3, level4, num_bits)
    error_weight = inject_error(error_weight, orig_weight, mlc_error_rate["error_level2"], level2, level3, num_bits)

    # Convert numpy weight to pytorch
    weight_float = error_weight.astype(np.float)
    weight_torch = torch.tensor(weight_float)
    weight_torch = weight_torch.reshape(shape)
    return weight_torch
