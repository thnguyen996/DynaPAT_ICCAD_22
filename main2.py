...
# Config weight and inject error:
    ...
    tensors =  (tensor_11, tensor_10, tensor_01, tensor_00)
    with torch.no_grad():
        count = 0
            ...
            for it in range(iteration):
                ...
                for (name, weight) in tqdm(state_dict.items(), desc="Executing method:", leave=False):
                    if ( "weight" in name ) and ( "bn" not in name ):
                        mlc_error_rate = {"error_lv3" : error_11, "error_lv2": error_10}
                        if args.method == "proposed_method":
                            error_weight = proposed_method(weight, weight_type, mlc_error_rate, name, encode=args.encode)
                        # if args.method == "flipcy":
                        #     error_weight = flipcy(weight, weight_type, mlc_error_rate, name, tensors, encode=args.encode)
                        # if args.method == "helmet":
                        #     error_weight = helmet(weight, weight_type, mlc_error_rate, name, tensors, encode=args.encode)
...
def proposed_method(weight, weight_type, mlc_error_rate, name, encode):
    #TODO: Implement flip method encoding
...
def flipcy(weight, weight_type, mlc_error_rate, name, tensors, encode):
    shape = weight.shape
    MLC = weight_conf(weight, weight_type)
    weight = cp.asarray(weight).ravel().view(cp.uint32)
    (tensor_11, tensor_10, tensor_01, tensor_00) = tensors
    if encode:
        encoded_weight = flipcy_en(MLC.weight)
        if not os.path.isdir(f"./flipcy_en/{args.model}-fix-code"):
            os.mkdir(f"./flipcy_en/{args.model}-fix-code")
        torch.save(encoded_weight, f"./flipcy_en/{args.model}-fix-code/{name}.pt")
        ...
        weight_torch = torch.tensor(float_weight, device="cuda").reshape(shape)
    else:
        assert os.path.isdir(f"./flipcy_en/{args.model}-fix-code"), "You need to do encoding first"
        encoded_weight = torch.load(f"./flipcy_en/{args.model}-fix-code/{name}.pt")
        # flip = torch.load(f"./flipcy_en/{args.model}/flip-{name}.pt")
        # comp = torch.load(f"./flipcy_en/{args.model}/comp-{name}.pt")
        num_11_flipcy, _ = count_11(encoded_weight, tensor_11)
        num_error_11_flipcy = int(mlc_error_rate["error_11"] * num_11_flipcy)
        num_10_flipcy, _ = count_10(encoded_weight, tensor_10, tensor_11)
        num_error_10_flipcy = int(mlc_error_rate["error_10"] * num_10_flipcy)
        num_11, _  = count_11(weight, tensor_11)
        num_10, _ = count_10(weight, tensor_10, tensor_11)
        num_error = (num_error_11_flipcy, num_error_10_flipcy)
        error_weight = inject_error(weight_type, weight, mlc_error_rate, num_error)
        float_weight = error_weight.view(cp.float32)
        weight_torch = torch.tensor(float_weight, device="cuda").reshape(shape)
    return weight_torch

def helmet(weight, weight_type, mlc_error_rate, name, tensors, encode):
    shape = weight.shape
    MLC = weight_conf(weight, weight_type)
    weight = cp.asarray(weight).ravel().view(cp.uint32)
    tensor_11, tensor_10, tensor_01, tensor_00 = tensors
    if encode:
        ...
        num_11_helmet, _ = count_11(encoded_weight, tensor_11)
        num_error_11_helmet = int(mlc_error_rate["error_11"] * num_11_helmet)
        num_10_helmet, _ = count_10(encoded_weight, tensor_10, tensor_11)
        num_error_10_helmet = int(mlc_error_rate["error_10"] * num_10_helmet)
        num_11, _  = count_11(weight, tensor_11)
        num_10, _ = count_10(weight, tensor_10, tensor_11)
        num_error = (num_error_11_helmet, num_error_10_helmet)
        error_weight = inject_error(weight_type, weight, mlc_error_rate, num_error)
        float_weight = error_weight.view(cp.float32)
        weight_torch = torch.tensor(float_weight, device="cuda").reshape(shape)
    return weight_torch

def count_11(weight, tensor_11):
    index_bit = cp.arange(0, 32, 2)
    num_11 = 0
    indices_11 = []
    for tensor_11_i, index_b in zip(tensor_11, index_bit):
        and_result = cp.bitwise_and(tensor_11_i, weight)
        index_11 = cp.stack((and_result == tensor_11_i).nonzero())
        bit_index = cp.full(index_11.shape[1], index_b)
        bit_index = cp.expand_dims(bit_index, 0)
        index_tensor = cp.concatenate((index_11, bit_index), axis=0)
    ...
    return num_11, total_index_11

def count_10(weight, tensor_10, tensor_11):
    index_bit = cp.arange(0, 32, 2)
    num_10 = 0
    indices_10 = []
    for tensor_10_i, tensor_11_i, index_b in zip(tensor_10, tensor_11, index_bit):
        and_result = cp.bitwise_and(tensor_11_i, weight)
        index_10 = cp.stack((and_result == tensor_10_i).nonzero())
        bit_index = cp.full(index_10.shape[1], index_b)
        bit_index = cp.expand_dims(bit_index, 0)
        index_tensor = cp.concatenate((index_10, bit_index), axis=0)
