...
def main():
    ...
    if save_data:
        df = pd.DataFrame(columns=["Time", "error_rate"])
            ...
            for name, weight in net.named_parameters():
                if ("weight" in name) and ("bn" not in name):
                    ...
                    if args.method == "proposed_method":
                        ...
                        error_quantized_weight, sum_error_0111_i, sum_0111_i = proposed_method(
                            quantized_weight, mlc_error_rate, args.num_bits, tensors, state_order
                        )
                    if args.method == "baseline":
                        error_quantized_weight, sum_error_0111_i, sum_0111_i = baseline(
                            quantized_weight, mlc_error_rate, args.num_bits, tensors
                        )
                    if args.method == "flipcy":
                        error_quantized_weight, sum_error_0111_i, sum_0111_i = flipcy(
                            quantized_weight,
                            mlc_error_rate,
                            name,
                            tensors,
                    ...
                    if args.method == "helmet":
                        error_quantized_weight, sum_error_0111_i, sum_0111_i = helmet(
                            quantized_weight,
                            mlc_error_rate,
                            name,
                            tensors,
                ...
                sum_error_0111 += sum_error_0111_i
                sum_0111 += sum_0111_i
            error_rate = sum_error_0111/sum_0111*100
            print(f"Error rate: {error_rate}")
            if save_data and not args.encode:
                df.loc[count] = [count, error_rate]
...
def proposed_method(weight, mlc_error_rate, num_bits, tensors, state_order):
    ...
    tensor_01_new = tensors[state_order[2]]
    tensor_11_new = tensors[state_order[3]]

    MLC.weight = MLC.weight.reshape(int(weight.size), 1)
    orig_weight = np.copy(MLC.weight)
    num_01 = flipcy_quan.count(orig_weight, tensor_01_new, tensor_11, num_bits)
    num_11 = flipcy_quan.count(orig_weight, tensor_11_new, tensor_11, num_bits)

    num_01 = np.sum(np.sum(num_01, axis=1), 0)
    num_11 = np.sum(np.sum(num_11, axis=1), 0)
    sum_0111 = num_01 + num_11
    ...
    sum_error_0111 = num_error_11 + num_error_01
    ...
    return weight, sum_error_0111, sum_0111
...
def flipcy(weight, mlc_error_rate, name, tensors, num_bits, encode):
    ...
    if encode:
        ...
        sum_error_0111 = num_error_01 + num_error_11
