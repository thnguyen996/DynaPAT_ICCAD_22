...
class weight_conf(object):
    def __init__(self, weight, num_bits):
        self.shape = weight.shape
        self.device = weight.device
        self.num_bits = num_bits
        self.weight = weight.view(-1).detach().cpu().numpy()
    ...
    def inject_error(self, mlc_error_rate, error_pat, des_pat):
        ...
        # Convert to self.num_bits bit floating point pytorch tensor
        weight_float = weight.astype(np.float)
