...
parser.add_argument(
    "--encode", action="store_true", help="Enable encode for flipcy and helmet")
...
def main():
    ...
    valset = datasets.ImageFolder(valdir, val_tf)
    ...
    if not os.path.isfile(f"./quant_stats/{args.arch}-imagenet.yaml"):
        caliset, _  = torch.utils.data.random_split(valset, [5000, 45000])
        ...
        with collector_context(collector):
            validate(cal_loader, model, criterion)
        yaml_path = f"./quant_stats/{args.arch}-imagenet.yaml"
        collector.save(yaml_path)
        print("Done")

    # Quantize the model
    assert os.path.isfile(f"./quant_stats/{args.arch}-imagenet.yaml"), "You should do activation calibration first"
    args.qe_stats_file = f"./quant_stats/{args.arch}-imagenet.yaml"
    ...
    quantizer.prepare_model(torch.randn(120, 3, 224, 224))
